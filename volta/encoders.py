...
class BertForVLTasks(BertPreTrainedModel):
    ...
    def forward(
        ...
        if self.task_cfg[task_id]["type"] == "V-logit":
            ...
            sequence_output_v_sample = [sequence_output_v[layer_idx] for layer_idx in self.task_cfg[task_id]["fuse_layers"]]
            #sequence_output_v_sample = sequence_output_v[self.task_cfg[task_id]["fuse_layers"]]
            vil_prediction = (pred_scores, layer_attn_scores, sequence_output_v_sample, image_attention_mask, fused_representation_v)
...
class ObjCategorizationClassifier(nn.Module):
    def __init__(self, t_hidden_size, embed_proj_size, latent_size, num_obj_classes, dropout_prob=0.1, num_clf_layers=2):
        super(ObjCategorizationClassifier, self).__init__()
        self.embed_proj_layer = nn.Linear(t_hidden_size, embed_proj_size)
        self.attn_proj_layer = nn.Sequential(
                nn.Linear(embed_proj_size, latent_size),
                nn.ReLU(),
                torch.nn.Dropout(dropout_prob),
    ...
    def compute_text_attentive_feature(self, sequence_output_t, attn_mask_t):
        """
        :param sequence_output_t: [batch, seq_len, embed_proj_size]
        :param attn_mask_t:
        :return:
        """
        # Compute self attention over text
...
class MultiLayerRoutingByAgreementFusionClassifier(nn.Module):
    def __init__(self, layer_indices, v_hidden_size, dropout_prob=0.1, num_clf_layers=1, num_capsules=32, num_iterations=3):
        ...
        assert self.v_hidden_size % self.num_capsules == 0
        ...
        self.V_projection = nn.ModuleList([nn.Linear(v_hidden_size, self.capsule_hidden_size, bias=False) for j in range(self.num_capsules)])
        #self.linears = nn.ModuleList([nn.Linear(num_layers * v_hidden_size, v_hidden_size) for i in range(num_layers)])
    ...
    def forward(self, sequence_output_v_all):
        ...
        batch_size, v_seq_len, num_layers, v_hidden_size = target_layers_tensor.size()
        #print("target_layers_tensor")
        #print(target_layers_tensor.size())
...
class MultiLayerPooledSelfAttnFusionClassifier(nn.Module):
    def __init__(self, layer_indices, v_hidden_size, dropout_prob=0.1, num_clf_layers=1):
        super(MultiLayerPooledSelfAttnFusionClassifier, self).__init__()
        if num_clf_layers == 1:
            self.clf = nn.Linear(v_hidden_size, 1)
        elif num_clf_layers == 2:
            self.clf = torch.nn.Sequential(
    ...
    def forward(self, sequence_output_v_all):
        ...
        layer_weights_normalized_expanded = layer_weights_normalized.unsqueeze(1)  # [batch, 1, num_layers, 1]
        ...
        #print(fused_representation_v.size())
        region_clf_logit = self.clf(self.dropout(fused_representation_v))  # [batch, v_seq_len, 1]
        return region_clf_logit, layer_weights_normalized.squeeze(2), fused_representation_v
...
class MultiLayerSelfAttnTextVisionFusionClassifier(nn.Module):
    def __init__(self, layer_indices, t_hidden_size, v_hidden_size, dropout_prob=0.1, num_clf_layers=1):
        ...
        # self.pre_classify_dropout = nn.Dropout(dropout_prob)
        print("MultiLayerSelfAttnTextVisionFusionClassifier built")
        print("Indices: ", layer_indices)
        print("No. of layers: ", num_layers)
...
class MultiLayerFineAttnFusionClassifier(nn.Module):
    def __init__(self, layer_indices, v_hidden_size, dropout_prob=0.1, num_clf_layers=1):
        super(MultiLayerFineAttnFusionClassifier, self).__init__()
        if num_clf_layers == 1:
            self.clf = nn.Linear(v_hidden_size, 1)
        elif num_clf_layers == 2:
            self.clf = torch.nn.Sequential(
